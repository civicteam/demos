...
export function McpStatus() {
  ...
  const copyToken = useCallback(async () => {
    if (!status?.tokenExchange.accessToken) return;
    try {
      await navigator.clipboard.writeText(status.tokenExchange.accessToken);
      setCopyState("copied");
      setTimeout(() => setCopyState("idle"), 2000);
    } catch (error) {
      console.error("Failed to copy token:", error);
    }
  }, [status?.tokenExchange.accessToken]);
  ...
  const getStatusState = (): StatusState => {
    ...
    if (status.tokenExchange.status === "success") {
      return "connected";
    }
  ...
  const getStatusText = (state: StatusState) => {
    switch (state) {
      ...
      case "connected":
        return "WebMCP ready";
      case "partial":
        return "Token exchange unavailable";
      case "disconnected":
      ...
      {expanded && status && (
        <div className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-4 z-50">
          ...
          {status.tokenExchange.accessToken && (
            <button
              onClick={copyToken}
              className="w-full mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-md transition-colors"
            >
              {copyState === "copied" ? (
                <>
          ...
          {state === "partial" && (
            <p className="text-xs text-gray-500 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
              Configure federated auth in Civic dashboard to enable WebMCP.
            </p>
          )}
        </div>
      )}
